...
function [] = SampleGenerator()
    ...
    if ~exist(fullfile(foldpath, "label"), 'dir')
        mkdir(fullfile(foldpath, "label"));
    end
    if ~exist(fullfile(foldpath, "molecular"), 'dir')
        mkdir(fullfile(foldpath, "molecular"));
    end
    if ~exist(fullfile(foldpath, "sample"), 'dir')
    ...
    generate_label(foldpath);          % paras.BitDepth

    % Generate samples
    generate_molecular_set(foldpath);  % double
    generate_sample(foldpath);         % paras.BitDepth
end
...
function [] = generate_label(foldpath)
...
function [] = generate_sample(foldpath)
    % load basic and sample parameters we will use
    load(fullfile(foldpath, "paras.mat"), "paras");
    NumMolecule = paras.NumMolecule;
    NumFrame    = paras.NumFrame;
