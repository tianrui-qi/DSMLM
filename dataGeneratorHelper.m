...
function paras = generateDataParas(paras)
    % Descriptions:
    % - This function will use input parameters to randomly generate
    %   data parameters that used to generate sample.
    % - Input parameters can help us generate datas with same
    %   distribution but not exatly same. To save a dataset, data
    ...
    % save data parameters in 'paras,' change dtype/rounded meantime
    paras.mu_set    = mu_set;
...
function [] = saveTif(path, frame)
    % Descriptions:
    % - This function will save the single 2D or 3D 'frame' as .tif in path
    % - Input frame normalized to [0, 1] and we will convert it to uint8
    %   in order to save it to .tif.
    % - Can be call indepedently. We use this function as help function for
    %   'saveFrames' that saves NumFrame number of frames.
    % Input:
    % - path: the path to save the 'frame,' does not contain .tif at the
    %   end.
    % - frame: a 2D or 3D frame normalized to [0, 1].
...
function [] = saveFrames(fold, subfold, frames)
    % Descriptions:
    % - This function will save NumFrame number of 2D or 3D frames in
    %   input 'frames' as .tif 
