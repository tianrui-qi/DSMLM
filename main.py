...
import src.cfg, src.data, src.model, src.runner
...
def main() -> None:
    ...
    args = getArgs()
    if args.mode == "evalu":
        config = src.cfg.ConfigEvaluer(**vars(args))
        src.runner.Evaluer(**config.runner,
            evaluset=src.data.RawDataset(**config.evaluset), 
        ).fit()
    if args.mode == "train":
        config = getattr(src.cfg, args.config)()
        src.runner.Trainer(**config.runner,
            trainset=src.data.SimDataset(**config.trainset), 
            validset=src.data.RawDataset(**config.validset), 
            model=src.model.ResAttUNet(**config.model), 
        ).fit()
...
def getArgs():
    ...
    getArgsTrain(subparsers.add_parser('train'))
    getArgsEvalu(subparsers.add_parser('evalu'))
...
def getArgsTrain(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "-config", type=str, required=True, dest="config",
        choices=[
            name for name, obj in inspect.getmembers(src.cfg) 
            if inspect.isclass(obj) 
            and issubclass(obj, src.cfg.ConfigTrainer) 
            and obj is not src.cfg.ConfigTrainer
        ],
        help="Choose a config class for training that inherit from " +
        "`src.cfg.ConfigTrainer`. Choices list all match object in `src.cfg`." + 
        "Config class is used instead of argparse since parameters may " + 
        "frequently add, remove, or change during model development and " + 
        "testing."
    )


def getArgsEvalu(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
